package ATrack::Accession;

=head1 NAME

ATrack::Accession - Assembly Tracking Accession object

=head1 SYNOPSIS
    my $accession = ATrack::Accession->new($atrack, $accession_id);

    my $id      = $accession->id();
    my $name    = $accession->name();
...
sub fields_dispatch
{
    ...
    return
    {
        accession_id => sub { $self->id(@_) },
        name      => sub { $self->name(@_) },
        secondary => sub { $self->secondary(@_) },
        type => sub { $self->type(@_) },  ## enum: project,run,asm,sample,specimen
        submission => sub { $self->submission(@_) },
        date => sub { $self->date(@_) },
        title => sub { $self->title(@_) },
        description => sub { $self->description(@_) },
    };
}
