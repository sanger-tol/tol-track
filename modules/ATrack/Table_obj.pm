package ATrack::Table_obj;

=head1 NAME

ATrack::Base_obj - Assembly Tracking Table_obj object

=head1 SYNOPSIS

=head1 DESCRIPTION

This is the superclass of all ATrack modules that represent a table in the
...
sub new
{
    my ($class, $atrack, $id, @other_args) = @_;
    ...
    if ($atrack)
    {
        confess "The interface has changed, expected atrack reference." unless $atrack && $atrack->isa('ATrack::ATrack');
        confess "Need to call with a atrack reference and id" unless ($atrack && $id);
    ...
    {
        confess "Need to call with a atrack reference and id";
    }
...
=head2 new_by_field_value
  ...
  Description: Class method. Returns changed $class object by field name and
...
{
    my ($class, $atrack, $field, $value, @other_args) = @_;
    confess "Need to call with a atrack handle, field name, field value" unless ($atrack && $field && defined $value);
    ...
    return unless $id;
    return $class->new($atrack, $id, @other_args);
}
...
=head2 create
  ...
  Example    : my $obj = $class->create($atrack, name => 'unique_name')
  Description: Class method. Creates new object in the database.
  Returntype : ATrack::Table_obj inheriting object

=cut

sub create
{
    my ($class, $atrack, %data) = @_;
...
=head2 update
  ...
  Description: Update a project whose properties you have changed.
...
=head2 _get_set_child_object
  ...
  Arg [3]    : identifier of child to find/create (optional; can be a list if
               Arg[1] takes a list)
  Example    : my $child = $obj->_get_set_child_object('get_project_by_accession',
                                                       'ATrack::Project',
                                                       'SRP000031');
...
sub _get_set_child_object
{
    ...
    if (! defined $self->{$child_storage_key})
    {
        ...
        {
            my $obj = $child_class->new($self->{atrack}, $self->$child_id_method);
            $self->{$child_storage_key} = $obj;
        }
    }
...
=head2 _create_child_object
  ...
  Arg [3]    : identifier of child to create (can supply list of ids as needed
               by Arg [1])
  Example    : my $child = $obj->_create_child_object('get_project_by_accession', 'ATrack::Project', 'SRP000031');
...
sub _create_child_object
{
    ...
    {
        $obj = $child_class->create($self->{atrack}, @child_identifiers);
...
=head2 _get_child_objects
  ...
  Example    : my $children = $obj->_get_child_objects('ATrack::Sample');
...
sub _get_child_objects
{
    ...
    unless ($self->{$children_storage_key})
    {
        ...
        {
            my $obj = $child_class->new($self->{atrack}, $id);
...
=head2 _get_child_ids
  ...
  Arg [1]    : class of the child
  Example    : my $children_ids = $obj->_get_child_ids('ATrack::Sample');
  Description: For internal use only by inheritors of Core_obj. Gets the child
...
sub _get_child_ids
{
    ...
    {
        my $history_sql = '';
        if ($child_class->isa('ATrack::Core_obj'))
        {
...
=head2 _add_child_object
  ...
  Example    : my $child = $obj->_add_child_object('new_by_name_project', 'ATrack::Sample', $sample_name, $project_id);
...
sub _add_child_object
{
    ...
    if ($existing_method)
    {
        ...
        my $obj = $child_class->$existing_method($self->{atrack}, @child_identifiers);
        if ($obj)
        {
            cluck "$child_class (@child_identifiers) is already present in the database\n";
            return;
    ...
    my $obj = $child_class->create($self->{atrack}, @child_identifiers);
    if ($obj)
    {
...
=head2 _all_values_by_field
  ...
  Example    ; ATrack::Table_obj->_all_values_by_field($atrack, 'name')
  Description: Class method. Probably best used internally by inheritors of
               Table_obj. Gives an array reference to all values in a column of
               the database table.
  Returntype : ArrayRef
...
sub _all_values_by_field
{
    ...
    confess "Need to call a atrack handle and field name" unless ($atrack && $field);
...
=head2 id
  ...
  Example    : my $id = $obj->id();
               $obj->id('104');
  Description: Get/Set for ID of a ATrack object
...
sub atrack
{
    ...
    if (defined $atrack && (! defined $self->{atrack} || "$atrack" ne "$self->{atrack}"))
    {
        $self->{_dbh} = $atrack->{_dbh};
        $self->{atrack} = $atrack;
    }
